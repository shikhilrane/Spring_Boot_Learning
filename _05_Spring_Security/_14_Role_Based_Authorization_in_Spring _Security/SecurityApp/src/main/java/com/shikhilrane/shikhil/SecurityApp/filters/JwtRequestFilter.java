    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
         try {
            ...
            if (userIdFromToken != null && SecurityContextHolder.getContext().getAuthentication() == null) { // Check if userId exists and no user is set in SecurityContext yet
                User userById = myUserDetailsService.getUserById(userIdFromToken);         // Fetch user details from database using the extracted userId
                UsernamePasswordAuthenticationToken authenticationToken = new UsernamePasswordAuthenticationToken(userById, null, userById.getAuthorities()); // Create authentication object for this user
